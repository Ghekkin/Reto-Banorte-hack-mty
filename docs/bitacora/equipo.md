...
## 2026-09-13, 00:10 — un `Revert "."` borró el trabajo de dos sesiones, y la recuperación

Al retomar el bug del input LIVE en Inicio apareció algo peor que el bug: **el trabajo de la
sesión anterior no estaba en el disco**. El commit `2a60a89 .` había juntado 78 archivos de
**dos sesiones distintas**, y `6e29175 Revert "."` los deshizo todos y ya está en `origin/main`.
Ficha: [issue del revert](../issues/2026-09-13-revert-punto-se-llevo-trabajo-ajeno.md).

Lo que hay que llevarse de aquí:

- **El diagnóstico inicial estuvo mal.** Se dio por hecho que el error `components[0] no es un
  objeto` era una regresión propia. Al abrir el archivo apareció el código viejo, y de ahí salió
  el revert. Leer el archivo antes de creerle a la propia memoria es lo que destapó el problema.
- **La recuperación fue por rutas, no revirtiendo el revert:** `git checkout 2a60a89 --
  pathspec-from-file`, 71 de los 78 archivos. Los siete de la otra sesión (`limitador.ts`,
  `lista-movimientos.tsx`, `agente.ts`, `cierre.ts`, `inicio/config.ts`,
  `inicio/generar.ts`) quedaron fuera a propósito: recuperar lo propio no debe pisar la
  decisión de quien revirtió, ni arrastrar media feature ajena.
- **Costura conocida:** `pantalla.ts` es mixto. Se restauró completo porque la validación de
  props resueltas vive ahí, y viene con un `normalizarListaDeComponentes` ajeno que **fabrica
  componentes a partir de su nombre**. Compila y las pruebas pasan, pero es una decisión de
  producto que nadie de esta sesión tomó. **Hay que confirmarla o quitarla.**
- **Los dos síntomas eran un solo bug:** el modelo serializó cada componente por separado, la
  portada no se pudo guardar, e Inicio se cayó a su respaldo — que es exactamente "solo la
  tarjeta de texto". Quedó fijado con dos pruebas en `agente.spec.ts`.
- **La cifra ya no se inventa:** con `simular_credito` de vuelta, pagar $5,500 al mes da
  **13 meses**; el modelo mostraba 11. El cruce que da confianza: pagando la mensualidad
  guardada de Ana ($4,570.95) la tool devuelve exactamente sus 15 meses de plazo.

Estado al cierre: **628 pruebas en verde**, `pnpm typecheck` limpio, `catalogo.json`
regenerado sin diff, y 25 tools en el MCP.
