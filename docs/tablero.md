...
| demo | parlack | Arreglado el "$0.00" reportado en `GastoPorCategoria`/`DetalleCategoria` al tocar "Ver Restaurantes": `pantalla.ts` pisaba un binding `{"path":...}` legítimo con relleno de demo. 3 pruebas nuevas, 721 pruebas del monorepo en verde. Issue #39 abierto para el mismo patrón en otros ~10 componentes (no tocados, cambio acotado). Chat de Maya narrando en vivo (`7d5f815`) y revisión de costos (#33 cerrado, #34 a medias, #37/#38 abiertos) de la sesión paralela. Sigue: `reiniciar-estado` + ensayo completo y `estable` | dom 06:21 |
## Bloqueos
