...
| mcp | aldair | Bloque A de `docs/arquitectura/orquestadores.md`: `analizar_gasto` + `analizar_ahorro` (O4, fachadas de lectura) y `ejecutar_decision` (O2, ciclo de acción de 3 pasos a 2). 21 tools en total, 117 pruebas. Bloque B (prefetch + que el prompt use `ejecutar_decision`) queda para quien tome `contrato` | sáb 11:15 |
| contrato | — | — | — |
...
- demo: **ensayo completo verificado (sáb 08:55)**, tiempos entre 2.9 s y 10.5 s; el estado de producción quedó limpio. Antes de `marcar-estable.sh` falta cerrar el issue **#9** (el reinicio de estado es el primer paso del checklist y hoy miente) y decidir qué hace Carmen: su chip "¿Cómo va mi portafolio?" no tiene componente en el catálogo y el agente improvisa con `MetaActiva`, pintando el valor del portafolio como si fuera una meta por alcanzar. Sigue: pitch (vacío), `vision-general.md` y `trade-offs.md` todavía en `plan`, y probar el plan B sin red
