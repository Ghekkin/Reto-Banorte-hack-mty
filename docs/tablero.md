...
| demo | parlack | **Ajustes en vivo** (`docs/como-funciona/ajustes-en-vivo.md`): fase 1 en `main` y ensayada con el modelo real (Ana: «¿y si pago $6,000?» cambia la tarjeta del crédito en su lugar y «Programar este pago» se aplica en la misma tarjeta, 3 de 3). Ahora: fase 2, gastos fuera del banco («le doy $2,000 a mi mamá» → la tarjeta de gasto cambia y «Guardar gasto»). Sigue: meta de ahorro y plan de tarjeta; `estable` tras el ensayo | dom 03:40 |
## Bloqueos
