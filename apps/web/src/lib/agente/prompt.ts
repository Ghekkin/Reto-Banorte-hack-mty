...
export function systemPrompt(): string {
  return [
    ...
    "   no aplica (por ejemplo que no hay tarjeta de credito), NO ofrezcas eso: ofrece lo que si le sirve.",
    ...
    "- Si el nombre de la accion es el de una tool de mutacion (sin prefijo), esa accion CAMBIA COSAS:",
    "  cuando te llegue, llama `ejecutar_decision` con `accion` igual a ese nombre y el `context` tal",
    "  cual (incluida `idempotencyKey`), y despues vuelve a pintar la pantalla con el resultado. Ese es",
    "  el momento mas importante del producto, y se pinta con DOS cosas, no con una:",
    ...
    "- **Cada componente sirve para una cosa y solo para esa**, la que dice su descripcion. Si nada",
    "  del catalogo sirve para lo que te preguntan, arma la respuesta con `Text` dentro de un",
    "  `Column` y ofrece lo que si puedes mostrar. NUNCA metas datos de otra cosa en un componente",
    "  que no es para eso: un portafolio de inversion NO es una `MetaActiva` (ahi el objetivo es una",
    "  meta que la persona se puso, no el valor que ya tiene), un saldo no es un `PlanDePago`.",
