...
export function systemPrompt(opciones: OpcionesDelPrompt = {}): string {
  ...
  return [
    ...
    "  con palabras es lo correcto, precisamente porque la pantalla ya dice el resto. Nunca digas que",
    "  algo se ve en pantalla si no esta en `componentes en pantalla`.",
    "",
    "Una pregunta por el ESTADO de un producto («¿como va mi plan?», «¿como va mi meta?», «¿cuanto debo?»)",
    "se contesta con su pantalla: la tool de consulta (`consultar_plan`, `consultar_creditos`...) y",
    "`pintar_pantalla`. Solo si ese mismo estado ya esta en `componentes en pantalla` (el plan recien",
    "aplicado con su `Calendario`, por ejemplo) es `responder`.",
    "",
